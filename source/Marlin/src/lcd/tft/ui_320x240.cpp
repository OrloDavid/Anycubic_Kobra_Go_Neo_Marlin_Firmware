...
// #####################################################################
  ...
  if (Heater >= 0) { // HotEnd
    if (currentTemperature >= 50) Color = COLOR_HOTEND;
      ...
      if (currentTemperature > 26) Color = COLOR_HOTEND;
...
//        tft.add_text(32, 6, color , tft_string); 
        ...
        tft.add_image(1, 1, imgSettings, COLOR_CONTROL_ENABLED);
		tft.add_text( 34, 8, COLOR_FOREGROUND , "Menu");  
...
// #####################################################################
  ...
  if(card_detected)
  tft.add_image(0, 0, imgSD, COLOR_SDCARD);
  else
  tft.add_image(0, 0, imgSD, COLOR_CONTROL_DISABLED);
...
// #####################################################################
       ...
       if(busy)    
       {
         color=COLOR_PAUSE_BTN;
       }
       else if(Paused)     
       {
         color=COLOR_PAUSE_ERROR; 
       }
       else         
       {
...
// #####################################################################
  ...
  bool  card_detected = card.isMounted();
  if(card_detected)
  tft.add_image(0, 0, imgSD, COLOR_CONTROL_ENABLED);
  else
  tft.add_image(0, 0, imgSD, COLOR_CONTROL_DISABLED);
...
// #####################################################################
  ...
  tft.add_text(tft_string.center(TFT_WIDTH), 1, COLOR_STATUS_MESSAGE, tft_string);
    ...
    TERN_(SDSUPPORT, add_control(0, 130, menu_media, imgSD, !printingIsActive(), COLOR_SDCARD, card.isMounted() && printingIsActive() ? COLOR_SDCARD : COLOR_CONTROL_DISABLED));
  #endif
}
...
// #####################################################################
      ...
      if(yesno)
      {
        ...
        tft.set_background(COLOR_BACKGROUND);
        tft.add_image(0, 0, imgCancel, COLOR_CONTROL_NSELECTED);
        
        tft.canvas(180, 140, 64, 64);
        tft.set_background(COLOR_BACKGROUND);
        tft.add_image(0, 0, imgConfirm, COLOR_CONTROL_CONFIRM);
      ...
      else
     {
        tft.canvas(60, 140, 64, 64);
        tft.set_background(COLOR_BACKGROUND);
        tft.add_image(0, 0, imgCancel, COLOR_CONTROL_CANCEL);
        
        tft.canvas(180, 140, 64, 64);
        tft.set_background(COLOR_BACKGROUND);
        tft.add_image(0, 0, imgConfirm, COLOR_CONTROL_NSELECTED);
     }

  #if ENABLED(TOUCH_SCREEN)
    add_control(48, TFT_HEIGHT - 64, CANCEL, imgCancel, true, yesno ? HALF(COLOR_CONTROL_NSELECTED) : COLOR_CONTROL_CANCEL);
    add_control(208, TFT_HEIGHT - 64, CONFIRM, imgConfirm, true, yesno ? COLOR_CONTROL_NSELECTED : HALF(COLOR_CONTROL_CONFIRM));
  #endif
}
...
// #####################################################################
  ...
  void MarlinUI::ubl_plot(const uint8_t x_plot, const uint8_t y_plot) {
    ...
    tft.set_background(COLOR_BACKGROUND);
    tft.add_rectangle(0, 0, GRID_WIDTH, GRID_HEIGHT, COLOR_FOREGROUND);
    ...
    for (uint16_t x = 0; x < (GRID_MAX_POINTS_X); x++)
      for (uint16_t y = 0; y < (GRID_MAX_POINTS_Y); y++)
        if (position_is_reachable({ ubl.mesh_index_to_xpos(x), ubl.mesh_index_to_ypos(y) }))
          tft.add_bar(1 + (x * 2 + 1) * (GRID_WIDTH - 4) / (GRID_MAX_POINTS_X) / 2, GRID_HEIGHT - 3 - ((y * 2 + 1) * (GRID_HEIGHT - 4) / (GRID_MAX_POINTS_Y) / 2), 2, 2, COLOR_PROGRESS_BAR);

    tft.add_rectangle((x_plot * 2 + 1) * (GRID_WIDTH - 4) / (GRID_MAX_POINTS_X) / 2 - 1, GRID_HEIGHT - 5 - ((y_plot * 2 + 1) * (GRID_HEIGHT - 4) / (GRID_MAX_POINTS_Y) / 2), 6, 6, COLOR_PROGRESS_BAR);
...
#define E_BTN_COLOR COLOR_FOREGROUND
#define X_BTN_COLOR COLOR_FOREGROUND
#define Y_BTN_COLOR COLOR_FOREGROUND
#define Z_BTN_COLOR COLOR_FOREGROUND
...
static void drawMessage(const char *msg) {
  ...
  tft.set_background(COLOR_BACKGROUND);
  tft.add_text(0, 0, COLOR_FOREGROUND, msg);
}
...
static void drawBtn(int x, int y, const char *label, intptr_t data, MarlinImage img, uint16_t bgColor, bool enabled = true) {
  ...
  tft.set_background(COLOR_BACKGROUND);
  tft.add_image(0, 0, imgBtn39Rounded, bgColor, COLOR_BACKGROUND, COLOR_CONTROL_DISABLED);
  ...
  if (label) {
    ...
    tft.add_image(0, 0, img, bgColor, COLOR_BACKGROUND, COLOR_CONTROL_ENABLED);
  }
...
// #####################################################################
  ...
  // aligned with x+
  drawBtn(xplus_x, y, "off", (intptr_t)disable_steppers, imgCancel, COLOR_FOREGROUND, !busy);
