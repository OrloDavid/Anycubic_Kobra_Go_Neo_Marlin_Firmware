 ...
 * Also implemented the Bzier velocity curve evaluation in ARM assembler,
...
uint32_t Stepper::advance_divisor = 0,
  ...
  int32_t __attribute__((used)) Stepper::bezier_B __asm__("bezier_B");    // B coefficient in Bzier speed curve with alias for assembler
  int32_t __attribute__((used)) Stepper::bezier_C __asm__("bezier_C");    // C coefficient in Bzier speed curve with alias for assembler
  uint32_t __attribute__((used)) Stepper::bezier_F __asm__("bezier_F");   // F coefficient in Bzier speed curve with alias for assembler
  uint32_t __attribute__((used)) Stepper::bezier_AV __asm__("bezier_AV"); // AV coefficient in Bzier speed curve with alias for assembler
  #ifdef __AVR__
    bool __attribute__((used)) Stepper::A_negative __asm__("A_negative"); // If A coefficient was negative
  #endif
  bool Stepper::bezier_2nd_half;    // =false If Bzier curve has been initialized or not
#endif
...
void Stepper::set_directions() {
...
#if ENABLED(S_CURVE_ACCELERATION)
  /**
   *  This uses a quintic (fifth-degree) Bzier polynomial for the velocity curve, giving
   *  a "linear pop" velocity curve; with pop being the sixth derivative of position:
   *  velocity - 1st, acceleration - 2nd, jerk - 3rd, snap - 4th, crackle - 5th, pop - 6th
   *
   *  The Bzier curve takes the form:
   *
   ...
   *  through t of the Bzier form of V(t), we can determine that:
   *
   ...
   *  the Bzier curve at each point:
   *
   *        V_f(t) = A*t^5 + B*t^4 + C*t^3 + F          [0 <= t <= 1]
   *
   * Floating point arithmetic execution time cost is prohibitive, so we will transform the math to
   * use fixed point values to be able to evaluate it in realtime. Assuming a maximum of 250000 steps
   * per second (driver pulses should at least be 2S hi/2S lo), and allocating 2 bits to avoid
   * overflows on the evaluation of the Bzier curve, means we can use
   *
   ...
   *
   *  For AVR, the precision of coefficients is scaled so the Bzier curve can be evaluated in real-time:
   ...
   *       // Calculate the Bzier coefficients
   *       if (v1 < v0) {
   *         A_negative = true;
   *         bezier_A = 6 * (v0 - v1);
   *         bezier_B = 15 * (v0 - v1);
  ...
  do {
    ...
    if (!nextMainISR) nextMainISR = block_phase_isr();  // Manage acc/deceleration, get next block
      ...
      if (is_babystep)                                  // Avoid ANY stepping too soon after baby-stepping
        NOLESS(nextMainISR, (BABYSTEP_TICKS) / 8);      // FULL STOP for 125s after a baby-step
